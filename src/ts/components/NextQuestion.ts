...
const NextQuestion = (questions: Question[]) => {
    ...
    const checkAnswers = (answer: string) => {
        let bullets = document.querySelectorAll(".bullets .spans span") as NodeListOf<HTMLSpanElement>;
        const submit = <HTMLButtonElement>document.querySelector(".submit-btn");
        const answers = document.getElementsByName("answer") as NodeListOf<HTMLInputElement>;
        let chosenAnswer = "";
        answers.forEach((el: HTMLInputElement) => {
            if (el.checked) {
